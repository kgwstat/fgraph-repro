...
Cov1 <- Cov1 / sum(diag(Cov1)/R)
Cov2 <- outer(grid, grid, Vectorize(CovFn2))
Cov2 <- Cov2 / sum(diag(Cov2)/R)
Cov3 <- t(Smt) %*% Cov2 %*% Smt / R^{2}
Cov3 <- Cov3 / sum(diag(Cov3)/R)
Cov4 <- outer(grid, grid, Vectorize(CovFn4))
Cov4 <- Cov4 / sum(diag(Cov4)/R)
Cov5 <- outer(grid, grid, Vectorize(CovFn5))
Cov5 <- Cov5 / sum(diag(Cov5)/R)
######################################################################
######################################################################
CovsReg <- list(Cov1, Cov2, Cov3, Cov4, Cov5)
