...
CovPACE <- function(X) {
  R <- 600
  Grid <- seq(from = 0, to = 1, length.out = R)
  Ly  <- lapply(1:nrow(X), function(k) {X[k,][!is.na(X[k,])]})
  Lt  <- lapply(1:nrow(X), function(k) {Grid[which(!is.na(X[k,]))]})
  C   <- fdapace::GetCovSurface(Ly, Lt, optns = list(nRegGrid = 120, methodBwCov = 'GCV'))
  Cov <- C$cov
  Cov
}
